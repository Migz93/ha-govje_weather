...
def _get_wind_speed_ms(force: str | None) -> float | None:
    """Map a Beaufort force string (e.g. 'F4') to m/s."""
    if not force:
        return None
...
def _parse_day_name_to_iso(day_name: str | None) -> str:
    ...
    if day_name == "Tomorrow":
        return (now + timedelta(days=1)).strftime("%Y-%m-%dT12:00:00")
    ...
    if day_name:
        ...
        if match:
            ...
            if month < now.month and month in (1, 2):
                year += 1
            try:
                return datetime(year, month, day, 12, 0, 0).strftime("%Y-%m-%dT12:00:00")
            except ValueError:
                pass
...
def _build_daily_forecast(day_data: dict[str, Any]) -> Forecast:
    ...
    precip = max(
        int(day_data.get("rainProbMorning") or 0),
        int(day_data.get("rainProbAfternoon") or 0),
        int(day_data.get("rainProbEvening") or 0),
    )
    ...
    wind_speed = _get_wind_speed_ms(day_data.get("windSpeed"))
...
class GOVJEWeatherEntity(CoordinatorWeatherEntity["GOVJEWeatherDataUpdateCoordinator"]):
    ...
    def native_wind_speed(self) -> float | None:
        ...
        val = today.get(key) or today.get("windSpeedMPH")
        if val is None:
            return None
        try:
            return float(val)
