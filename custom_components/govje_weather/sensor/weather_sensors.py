...
def _get_native_value(key: str, data: dict[str, Any]) -> Any:
    ...
    if key == "wind_speed_mph":
        return _as_int(today.get(f"windspeedMph{period}"))

    if key == "wind_speed_knots":
        return _as_int(today.get(f"windspeedKnots{period}"))

    if key == "wind_speed_kph":
        return _as_int(today.get(f"windspeedKM{period}"))
    ...
    if key == "rain_probability":
        return _as_int(today.get(f"rainProb{period}"))
