...
This document describes the technical architecture of the GOVJE Weather custom component for Home Assistant.
...
│   ├── __init__.py          # Exports GOVJEWeatherDataUpdateCoordinator
...
│   ├── __init__.py          # Exports GOVJEWeatherEntity
...
- `base.py` - Main coordinator class (`GOVJEWeatherDataUpdateCoordinator`)
...
**Key class:** `GOVJEWeatherDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `GOVJEWeatherApiClient`
...
- `GOVJEWeatherConfigFlowHandler` (main flow)
- `GOVJEWeatherOptionsFlow` (options)
...
**Key class:** `GOVJEWeatherEntity` (in `entity/base.py`)
...
2. `GOVJEWeatherEntity` for common functionality
...
1. Create directory: `custom_components/govje_weather/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `GOVJEWeatherEntity`
