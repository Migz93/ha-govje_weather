...
This document describes all configuration options and settings available in the GOVJE Weather custom integration.
...
```yaml
service: govje_weather.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: govje_weather.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/Migz93/ha-govje_weather/issues) - Report problems
