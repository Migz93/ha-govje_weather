...
**Directory:** `custom_components/govje_weather/[platform]/`
...
```python
"""[Platform] platform for GOVJE Weather."""
...
from .entity import GOVJEWeatherEntity
from .[entity_file] import GOVJEWeather[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: GOVJEWeatherDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            GOVJEWeather[EntityName](coordinator, entry),
...
```python
"""[Entity description] for GOVJE Weather."""
...
Add platform to `custom_components/govje_weather/manifest.json`:
...
- [ ] Entity class inherits from both `GOVJEWeatherEntity` and platform class
...
- **Base entity:** `GOVJEWeatherEntity` in `entity/base.py`
- **Coordinator:** `GOVJEWeatherDataUpdateCoordinator`
